...
Elm.Main.make = function (_elm) {
   ...
   $Json$Decode.string)),
   "http://localhost:8080");
   var helloWorldRunner = Elm.Native.Task.make(_elm).perform(function () {
      var errorLog = function (error) {
         return $Debug.log("error")(function () {
            switch (error.ctor)
            {case "BadResponse":
               return A2($Basics._op["++"],
                 "bad respose, code : ",
      ...
      return $Task.mapError(errorLog)($Task.map($Debug.log("respose"))(helloWorldTask));
   }());
